...
if ! [[ $REMOTE = *"authz.pro"* || -n $CI ]] ; then
    echo
    echo -e "\e[7m\033[33mNotice\033[0m"
    echo -e "Fork detected. If redistributing, generate your own API keys in ./src/models/credentials.ts"
    echo
fi
...
if [[ $PLATFORM = "prod" ]]; then
    ...
    ./node_modules/.bin/tsc --target ES2015 --esModuleInterop --moduleResolution nodenext --module commonjs scripts/test-runner.ts || true
else
    ./node_modules/webpack-cli/bin/cli.js
fi
./node_modules/sass/sass.js sass:css
cp ./sass/DroidSansMono.woff2 ./sass/Manrope.woff2 ./sass/mocha.css ./css/
